function tissue_prob = linear_tissue_prob_HighNeuronThresh(img, neuron_hist_thresh)
...
% want to add a nonlinearity
    ...
    frac_below = cumsum(counts) / sum(counts); % fraction of pixels below edges(2:end)
    neuron_idx = find( frac_below > neuron_hist_thresh, 1, 'first' );
    neuron.I_thresh = 10 ^ edges( neuron_idx + 1 ); % upper 5 percentile of pixel intensities
